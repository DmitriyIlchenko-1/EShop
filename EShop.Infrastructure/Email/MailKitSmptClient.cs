...
public class MailKitSmtpClient : Disposable, ISmtpClient
{
    ...
    public async Task ConnectAsync()
    {
        try
        {
            // TODO: this is a temporary fix because Google's smtp certificate became incomplete all of a sudden. This does need real fixing, though.
            _client.ServerCertificateValidationCallback = (_, _, _, _) => true;
            await _client.ConnectAsync(_mailAccount.Host,
                _mailAccount.Port,
                (SecureSocketOptions)_mailAccount.SecureMailOptions);
    ...
    public async Task SendAsync(string subject, string from, string to, string message,
